...
<html>
	...
	<?php
		require('conn.php');
		$NUMBER = $_GET['REQ'];
			$dateupload = "SELECT bill_month FROM tbl_log_csv_debt2 where right(bill_month,4) = YEAR(CURRENT_DATE)+543 and region = '$NUMBER' ORDER BY bill_month DESC LIMIT 1";
		...
		//$NUMBER2 = $_GET['REQ2'];
		$sql = "SELECT count(DISTINCT cus_number) as num, debtor_kpi.dept_name, debtor_kpi.sap_code from debtor_kpi join pea_office on pea_office.sap_code = debtor_kpi.sap_code where region2 LIKE '$NUMBER' GROUP BY debtor_kpi.sap_code";
	...
	?>
		<div data-role="page" id="page">
			...
			<?php 
				...
				echo "<b>รายงานลูกหนี้ค่าไฟฟ้าเอกชนรายใหญ่ค้างชำระเกินกำหนดรวมค่าไฟฟ้าเดือน $mmm แยกตามสังกัดของ $reg ข้อมูล ณ วันที่ $ccc</b><br/>";
			?>
			</div>
			<div data-role="content">	
