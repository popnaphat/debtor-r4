...
<html>
	...
	<?php
		require('conn.php');
		$NUMBER = $_GET['REQ'];
			$dateupload = "SELECT bill_month FROM tbl_log_csv_debt2 where region = '$NUMBER' AND right(bill_month,4) = (SELECT MAX(RIGHT(bill_month,4)) FROM tbl_log_csv_debt2 where region = '$NUMBER') ORDER BY left(bill_month,2) DESC LIMIT 1";
	...
	?>
		<div data-role="page" id="page">
			...
			<?php 
				...
				echo "<b>รายงานลูกหนี้ค่าไฟฟ้าเอกชนรายใหญ่ค้างชำระเกินกำหนด (บิลค้างชำระเดือนล่าสุด $mmm) แยกตามสังกัดของ $reg ข้อมูล ณ วันที่ $ccc</b><br/>";
			?>
			</div>
			<div data-role="content">	
