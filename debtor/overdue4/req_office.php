...
<html>
	...
	<?php
		...
		$dateupload = "SELECT bill_month FROM tbl_log_csv_debt5 where right(bill_month,4) = YEAR(CURRENT_DATE) and region = '$reg' ORDER BY bill_month DESC LIMIT 1";
			...
			$crecord2 = "SELECT * FROM tbl_log_csv_debt5 ORDER BY id DESC LIMIT 1";
			$crecord1 = mysqli_fetch_array(mysqli_query($conn,$crecord2));
			$ccc = $crecord1['file_upload_timestamp'];
		
		$sql = "SELECT * from debtor_kpi4 where sap_code = '$NUMBER'";
		$query = mysqli_query($conn,$sql) or die(mysqli_error($conn));
		$mode2 = mysqli_num_rows($query);
		
		$sql2 = "SELECT * from debtor_kpi4 k join pea_office o on o.sap_code = k.sap_code where k.sap_code = '$NUMBER' GROUP BY k.cus_number";
	...
	?>
		<div data-role="page" id="page">
			...
			<?php 
				echo "<b>รายงานลูกหนี้ค่าไฟฟ้าเอกชน-รายย่อยหลังโอนหักเงินประกันการใช้ไฟ ก่อนปี $curyear ของ $ofname1 จำนวน $mode1 ราย $mode2 บิล ลูกหนี้ฯคงเหลือ ณ วันที่ $ccc</b><br/>";	
			...
			<?php
					...
					while($result = mysqli_fetch_array($query)){
							...
							echo "<b>".$aa.".</b>".$result["cus_name"].", <b>หมายเลขผู้ใช้ไฟ:</b>".$result["cus_number"].", <b>บิลเดือน:</b>".$result["bill_month"].", <b>หนี้ค้าง:</b>".$result["outstanding_debt"]."บาท, <b>โอนหักเดือน:</b>".$result["transfer_month"].", <b>คำสั่ง:</b>".$result["pea_command"].", <b>สถานะ:</b>".$result["status"]."<br>";
							echo '</div>';						
						$aa = $aa+1;
					}
					mysqli_data_seek($query,0);
