...
<html>
	...
	<?php
		...
		//$NUMBER2 = $_GET['REQ2'];
		$dateupload = "SELECT bill_month FROM tbl_log_csv_debt4 where right(bill_month,4) = YEAR(CURRENT_DATE) and region = '$reg' ORDER BY bill_month DESC LIMIT 1";
		...
		$sql = "SELECT * from debtor_kpi3 where sap_code = '$NUMBER'";
		$query = mysqli_query($conn,$sql) or die(mysqli_error($conn));
		$mode2 = mysqli_num_rows($query);
		
		$sql2 = "SELECT * from debtor_kpi3 k join pea_office o on o.sap_code = k.sap_code where k.sap_code = '$NUMBER' GROUP BY k.cus_number";
		...
		<div data-role="page" id="page">
			...
			<?php
					...
					while($result = mysqli_fetch_array($query)){
							...
							echo "<b>".$aa.".</b>".$result["cus_name"].", <b>หมายเลขผู้ใช้ไฟ:</b>".$result["cus_number"].", <b>สายการอ่านหน่วย:</b>".$result["line_code"].", <b>บิลเดือน:</b>".$result["bill_month"].", <b>ประเภทเอกสาร:</b>".$result["doc_type"].", <b>หนี้ค้าง:</b>".$result["outstanding_debt"]."บาท<br>";
							echo '</div>';						
						$aa = $aa+1;
					}
					mysqli_data_seek($query,0);
